// DIGITALIUM.IO — Institutional: Organisation (Lecture seule)
...
import {
  ...
  Users, FolderTree, Layers3, Package, 
...
const ORG_TYPE_GRADIENTS: Record<string, string> = {
  enterprise: "from-violet-600 to-indigo-500",
  institution: "from-amber-500 to-orange-500",
  government: "from-emerald-500 to-teal-500",
  organism: "from-cyan-500 to-teal-500",
  platform: "from-red-500 to-orange-500",
};
...
function InfoRow({ icon: Icon, label, value, accent = false }: {
  ...
  return (
    <div className="flex items-start gap-3 py-2.5">
      <div className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 ${accent ? "bg-amber-500/10" : "bg-white/5"}`}>
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Badge className={`${ORG_TYPE_COLORS[(org.type as string) || "institution"]} border-0 text-[10px]`}>
                {ORG_TYPE_LABELS[(org.type as string) || "institution"]}
              </Badge>
...
const MODULE_META: Record<string, { label: string; gradient: string; description: string }> = {
  idocument: { label: "iDocument", gradient: "from-blue-500 to-blue-600", description: "Gestion documentaire intelligente" },
  iarchive: { label: "iArchive", gradient: "from-emerald-500 to-emerald-600", description: "Archivage numérique conforme OHADA" },
  isignature: { label: "iSignature", gradient: "from-violet-500 to-violet-600", description: "Signature électronique sécurisée" },
  iasted: { label: "iAsted", gradient: "from-amber-500 to-amber-600", description: "Assistant IA documentaire" },
};
      ...
      <Card className="glass border-white/5">
        <CardContent className="flex flex-col items-center py-12">
          <div className="h-12 w-12 rounded-xl bg-amber-500/10 flex items-center justify-center mb-3">
            <Info className="h-6 w-6 text-amber-400/60" />
          </div>
...
export default function InstitutionalOrganizationPage() {
  ...
  const gradient = ORG_TYPE_GRADIENTS[orgType] || ORG_TYPE_GRADIENTS.institution;
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <motion.div
        ...
        className="flex items-center gap-3 px-4 py-2.5 rounded-lg border border-amber-500/20 bg-amber-500/5"
      >
        <Eye className="h-4 w-4 text-amber-400 shrink-0" />
        <p className="text-xs text-muted-foreground">
          <span className="font-medium text-amber-300">Mode lecture</span> — Les modifications sont gérées par l'administrateur DIGITALIUM via le back-office.
        </p>
      </motion.div>
