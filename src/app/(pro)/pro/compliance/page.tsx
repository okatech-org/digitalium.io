...
// ═══════════════════════════════════════════════
// DIGITALIUM.IO — Page: Conformité Pro
// Sécurité & conformité réglementaire (Entreprise)
// ═══════════════════════════════════════════════
...
const complianceMetrics = [
    { label: "Score global", value: "92%", icon: Shield, color: "text-blue-500", bgColor: "bg-blue-500/10" },
    { label: "Documents conformes", value: "1,240", icon: FileCheck, color: "text-indigo-500", bgColor: "bg-indigo-500/10" },
    { label: "Alertes actives", value: "3", icon: AlertTriangle, color: "text-amber-500", bgColor: "bg-amber-500/10" },
    { label: "Audit trail", value: "100%", icon: Activity, color: "text-cyan-500", bgColor: "bg-cyan-500/10" },
];

const complianceChecks = [
    { label: "Rétention documentation fiscale", status: "pass", detail: "Les archives de facturation respectent les 10 ans légaux" },
    { label: "Signatures numériques valides", status: "warning", detail: "3 signatures en attente de renouvellement de certificat" },
    { label: "Chiffrement au repos", status: "pass", detail: "AES-256 actif sur tous les espaces documentaires" },
    { label: "Contrôle d'accès RBAC", status: "pass", detail: "Vérification des droits d'accès à jour" },
    { label: "Expiration certificats SSL", status: "pass", detail: "Certificats valides pour les prochains 90 jours" },
    { label: "Sauvegarde off-site", status: "pass", detail: "Dernière sauvegarde : il y a 2 heures" },
    { label: "Conformité RGPD", status: "pass", detail: "Registre des traitements conforme" },
    { label: "Journalisation des accès", status: "warning", detail: "Taille des journaux critiques élevée" },
];
...
const auditLog = [
    { action: "Connexion admin", user: "DG Ascoma", time: "Il y a 5 min", type: "info" },
    { action: "Document partagé", user: "Commercial", time: "Il y a 15 min", type: "success" },
    { action: "Tentative d'accès hors réseau", user: "Inconnu", time: "Il y a 45 min", type: "warning" },
    { action: "Archive vérifiée", user: "Système", time: "Il y a 1h", type: "info" },
    { action: "Contrat validé", user: "Juridique", time: "Il y a 3h", type: "success" },
];
...
export default function ProCompliancePage() {
    ...
    const handleGenerateReport = async () => {
        ...
        toast.info("Analyse de la conformité en cours...");
            ...
            >
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button 
                        ...
                        className="text-xs h-9 bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 border border-blue-500/30 transition-all"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-4">
                ...
                <Card className="glass border-white/5 flex flex-col h-full hover:border-white/10 transition-colors">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-semibold">Contrôles de sécurité en temps réel</CardTitle>
                        <CardDescription className="text-[11px]">Surveillance des standards RGPD & financiers</CardDescription>
                    </CardHeader>
                        ...
                        {isGenerating && (
                            <div className="absolute inset-0 bg-background/50 backdrop-blur-[1px] z-10 flex items-center justify-center rounded-lg">
                                <div className="flex flex-col items-center gap-2">
                                    ...
                                    <span className="text-xs font-medium text-blue-400">Analyse des standards en cours...</span>
                                </div>
                            </div>
                        )}
                            ...
                            {complianceChecks.map((check, i) => {
                                ...
                                return (
                                    <motion.div
                                        ...
                                        className="flex items-start gap-3 p-3 rounded-lg border border-white/5 hover:border-blue-500/10 hover:bg-blue-500/[0.02] transition-colors"
                                    >
                ...
                <Card className="glass border-white/5 flex flex-col h-full hover:border-white/10 transition-colors">
                    <CardHeader className="pb-2">
                        ...
                        <CardDescription className="text-[11px]">Traçabilité des accès et actions</CardDescription>
                    </CardHeader>
