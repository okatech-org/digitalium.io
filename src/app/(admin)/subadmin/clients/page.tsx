// DIGITALIUM.IO — SubAdmin: Clients
"use client";
import React from "react";
import { motion } from "framer-motion";
import { UserCircle, Plus, Search, Mail, Phone, Building2 } from "lucide-react";
    ...
    return (
        <motion.div initial="hidden" animate="visible" variants={stagger} className="space-y-6 max-w-[1400px] mx-auto">
            <motion.div variants={fadeUp} className="flex items-center justify-between">
                <div><h1 className="text-2xl font-bold flex items-center gap-2"><UserCircle className="h-6 w-6 text-violet-400" /> Clients</h1><p className="text-sm text-muted-foreground mt-1">{CLIENTS.length} clients</p></div>
            ...
            <motion.div variants={fadeUp} className="glass-card rounded-2xl p-5 overflow-x-auto">
                <table className="w-full text-xs">
                    <thead><tr className="border-b border-white/5 text-muted-foreground"><th className="text-left py-2 px-2">Client</th><th className="text-left py-2 px-2">Type</th><th className="text-left py-2 px-2 hidden md:table-cell">Contact</th><th className="text-right py-2 px-2 hidden sm:table-cell">Documents</th><th className="text-right py-2 px-2">Revenue</th></tr></thead>
                    <tbody>
                        {CLIENTS.map((c, i) => (
                            <tr key={i} className="border-b border-white/5 hover:bg-white/[0.02]">
                                <td className="py-2.5 px-2 font-medium">{c.name}</td>
                                <td className="py-2.5 px-2"><Badge variant="secondary" className="text-[9px] bg-white/5 border-0">{c.type}</Badge></td>
