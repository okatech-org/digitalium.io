...
export default function ISignaturePage() {
    ...
    const rawSignatures = useQuery(api.signatures.listByOrganization, orgId ? { organizationId: orgId as Id<"organizations"> } : "skip");
    ...
    const requests: SignatureRequest[] = useMemo(() => {
        ...
        return rawSignatures.map((sig) => {
            ...
            if (sig.status === "completed") {
                ...
                folderId = "sent"; // Default for admins monitoring org requests if not explicitly to_sign
            }
    ...
    const handleFileClick = useCallback((fileId: string) => {
        router.push(`/institutional/isignature/${fileId}`);
    }, [router]);
