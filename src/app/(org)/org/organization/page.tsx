// DIGITALIUM.IO — Organisme: Organisation (Lecture seule)
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Badge className={`${ORG_TYPE_COLORS[(org.type as string) || "organism"]} border-0 text-[10px]`}>
                {ORG_TYPE_LABELS[(org.type as string) || "organism"]}
              </Badge>
...
export default function OrgOrganizationPage() {
  ...
  const gradient = ORG_TYPE_GRADIENTS[orgType] || ORG_TYPE_GRADIENTS.organism;
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <motion.div
        ...
        className="flex items-center gap-3 px-4 py-2.5 rounded-lg border border-violet-500/20 bg-violet-500/5"
      >
        <Eye className="h-4 w-4 text-violet-400 shrink-0" />
        <p className="text-xs text-muted-foreground">
          <span className="font-medium text-violet-300">Mode lecture</span> — Les modifications sont gérées par l'administrateur DIGITALIUM via le back-office.
        </p>
      </motion.div>
